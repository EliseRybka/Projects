...
class LogRegL2Oracle(BaseSmoothOracle):
    ...
    def func(self, x):
        # TODO: Implement
        m = self.b.size
        return (1/m) * (np.log(np.ones(m) + np.exp(-self.b * self.matvec_Ax(x)))).dot(np.ones(m)) + self.regcoef / 2 * x.dot(x)
...
def create_log_reg_oracle(A, b, regcoef, oracle_type='usual'):
    ...
    matvec_ATx = lambda x:  A.transpose().dot(x)  # TODO: Implement

    def matmat_ATsA(s):
        # TODO: Implement
        #За пояснения в реализации спасибо Шибаеву Инокентию
        if (scipy.sparse.isspmatrix_csr(A)):
            return (A.T).dot(scipy.sparse.csr_matrix(np.diag(s)).dot(A))
        else:
