...
const function_code FUNCTION_CODES [NUMBER_OF_FUNCTIONS] = {{SQRT, "sqrt"}, {EXP, "exp"}, 
                         {LN, "ln"}, {SIN, "sin"}, {COS, "cos"}, {TG, "tg"}, {CTG, "ctg"},   
        ...
        switch (lexema -> type)
        {
                ...
                case OPERATOR:
                {
                        switch ((int) lexema -> value)
                        {
                                ...
                                case '^':
                                {
                                        if (left -> lexema -> type == NUMBER or
                                                left -> lexema -> type == VARIABLE)
...
double read_number (string formula, int *position)
{
        ...
        double result = signum * result_ignore_point / pow (10, num_AP);
        return result;
}
